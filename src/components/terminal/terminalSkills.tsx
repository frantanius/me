...
const TerminalSkills = ({
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data }: ISkillCategoriesResponse = await res.json();
    ...
    return (
      <>
        {data
          ...
          .map((skillCategory: ISkillCategory) => (
            <div key={uuidv4()} data-testid="skill-category">
              ...
              <ul className="mb-1 pl-4">
                {skillCategory.attributes.skills.data.map((skill: ISkill) => (
  ...
  return (
    <>
      {data?.map((skillCategory: ISkillCategory) => (
        <div key={uuidv4()} data-testid="skill-category">
